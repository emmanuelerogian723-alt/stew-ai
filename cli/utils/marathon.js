/**
 * Stew Code Marathon Mode — run autonomously for hours, not minutes.
 *
 * The difference from Agent Mode: Agent Mode plans once and executes a
 * fixed step list. Marathon Mode RE-PLANS continuously — it keeps working
 * toward the goal, checkpointing progress to disk, recovering from errors,
 * and only stops when the goal is verifiably done, the time budget runs
 * out, or the user drops a stop file. It survives crashes: kill the
 * process and resume the exact session later with --resume.
 *
 * Neither Claude Code's auto/yolo mode nor OpenCode offer a checkpointed,
 * resumable, time-boxed autonomous loop like this out of the box.
 */
const fs = require('fs');
...
/**
 * Ask the AI: is the goal complete based on what's been done so far?
...
/**
 * Plan the next batch of steps given current progress.
 */
async function planNextBatch(client, goal, log, cwd, focus) {
  ...
  prompt += '{"action": "read|write|shell|search|skill|analyze", "description": "...", "target": "...", "code": "... (write only)"}\n';
...
/**
 * Run marathon mode. Options:
 * - maxHours: time budget (default 4)
 * - maxIterations: batch-plan iterations budget (default 200)
 * - resumeId: resume an existing session
 * - onUpdate: callback(session) for live status (e.g. periodic broadcast)
 */
async function runMarathon(client, goal, cwd, options) {
  ...
  while (session.iteration < maxIterations) {
    ...
    // Heartbeat + completion check every N iterations
    ...
    if (steps.length === 0 && session.iteration > 1) {
      // Nothing to do and not complete — avoid infinite spin
