// Node's fetch always sets err.message = 'fetch failed' and hides the real reason in err.cause.
// This turns that black box into an actionable diagnosis (DNS, TLS/cert, connection reset, etc).
function describeFetchError(err) {
  var cause = err && err.cause;
  var code = cause && (cause.code || (cause.cause && cause.cause.code));
  var base = (err && err.message) || 'Network request failed';
  var hints = {
    ENOTFOUND: 'DNS lookup failed — check your internet connection or the domain/URL',
    EAI_AGAIN: 'DNS lookup timed out — check your internet connection',
    ECONNREFUSED: 'Connection refused — the server is down, unreachable, or blocked by a firewall',
    ECONNRESET: 'Connection reset — often means a firewall, proxy, or anti-bot system blocked the request',
    ETIMEDOUT: 'Connection timed out — slow/unreachable network',
    CERT_HAS_EXPIRED: 'TLS certificate expired',
    UNABLE_TO_VERIFY_LEAF_SIGNATURE: 'TLS certificate chain could not be verified — your system may be missing root CA certificates (common on minimal Termux/proot/Docker setups). Try: apt-get install -y ca-certificates',
    UNABLE_TO_GET_ISSUER_CERT_LOCALLY: 'Missing root CA certificates. Try: apt-get install -y ca-certificates',
    SELF_SIGNED_CERT_IN_CHAIN: 'A self-signed certificate is in the chain (often a captive portal or intercepting proxy)',
    DEPTH_ZERO_SELF_SIGNED_CERT: 'Self-signed certificate — often a captive portal (e.g. hotel/airport WiFi login page) intercepting HTTPS',
  };
  if (code && hints[code]) return base + ': ' + code + ' — ' + hints[code];
  if (code) return base + ': ' + code + (cause.message ? ' (' + cause.message + ')' : '');
  if (cause && cause.message) return base + ': ' + cause.message;
  return base;
}
...
module.exports = { StewError, Search, Skills, Documents, FineTune, Chat, describeFetchError };
